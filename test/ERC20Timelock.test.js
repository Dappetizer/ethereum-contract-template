...
contract("ERC20Timelock Unit Tests", async (accounts) => {
  ...
  before(async () => {
    ...
    this.contracts[0] = await ERC20TestContract.new(tokenName, tokenSymbol, {
      from: deployer,
    });
  ...
  it("Can deploy contract", async () => {
    ...
    //deploy contract
    this.contracts[1] = await ERC20TimelockContract.new(
      tokenAddress,
      beneficiaryAddress,
      lockDuration
    );
  ...
  it("Can receive tokens", async () => {
    ...
    const t1 = await this.contracts[0].transfer(lockAddress, lockAmount, {
      from: userA,
    });
